...
Vue.component('pre-publication', {
  ...
  publicationDraft(formData) {   
    ...
    if (this.publication_type=='Twitter') {
      res={title:'',date:new Date().toLocaleDateString(),summary:'',text:formData.get("text"),notes:formData.get("notes"),draft:formData.get("draft"),image:formData.get("image"),type:this.publication_type,screen_name:this.fscreen_name,webDate:formData.get("webDate")}
    }
    else{
