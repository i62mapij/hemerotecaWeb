//component that shows the list of publications and allows the capture of new publications
Vue.component('web-catch',{
    props: {
        publication_type: '',
        type_screen:''
    },
    ...
    mounted(){
        //when it mounts the page it calls the function that gets publications
        this.callServerForPublications();
    },
    methods:{
        //call apirest to get a token and call the get publications function
        ...
        getPublications: function(){
            //call apirest to get the list of publications of the type
            let headers = new Headers()
     
            headers.set('Authorization', 'Bearer '+this.clientToken)
            fetch('/api/publications/'+this.publication_type+'/'+this.page, { headers:headers })
            .then(res => res.json())
            .then(res => { 
                if(res.code == 200){
                 this.publications = res
                 //the last element of the array is de pagination object
        ...
        callDelete(formData){
            //function to delete publications calling to the rest api
            let headers = new Headers()
            headers.set('Authorization', 'Bearer '+this.clientToken)
        ...
        publicationAdd: function(){
            this.publicationSelected = undefined
        ...
        eventPublicationInsert: function(publication){
            if (publication.draft=='S'){
             this.publications.data.unshift(publication)
            }
            else{
             this.callServerForPublications();
            } 
        },
        eventPrePublication: function(publication){
            this.publicationSelected=publication
            this.timePre = new Date().getTime()
        },
        eventTuitsList: function(screen_name){
            this.timeTuits = new Date().getTime()
        ...
        eventPrePublicationTweet: function(tuit){
           this.timePreTweet = new Date().getTime()
           this.urlTweet=tuit.url
           this.screen_name=tuit.screen_name
        },
    ...
    `
        <div>
            ...
            <div v-if="this.publication_type!='Telegram'" align="right"><button class="btn btn-primary" v-on:click="publicationAdd">Añadir nueva</button></div>
            <div v-if="publications.length == 0">
                <h1>NO hay publicaciones</h1>
            </div>            
            <div v-else>
              <h2>Últimas publicaciones en {{publication_type}}
             </h2>
            </div>
            <pagination  v-on:eventPagination="eventPagination" :objectpagination="this.objectPagination"></pagination>
    
            <div class="scrollPanelCatcher">
             <div v-for="(publication, index) in publications.data" class="jumbotron pb-2 pt-3"> 
                    <h6>
                        <a :href="publication.url" target="_blank">
                        <img v-if="publication.image" :src="publication.image"  alt="..." class="float-left" width="10%" height="10%">
                   ...
                   <div v-if="publication.type=='web'"><i class="fab fa-google" title="Publicación web"></i></div> 
                ...
                <get-source  v-on:eventShowLoading="eventShowLoading" v-on:eventTuitsList="eventTuitsList" v-on:eventPrePublication="eventPrePublication"  v-on:eventPublicationUpdate="eventPublicationUpdate" v-on:eventPublicationInsert="eventPublicationInsert" :publicationEdit="publicationSelected" :time="timeSave" :publication_type=this.publication_type :timePreTweet="timePreTweet" :url="urlTweet" :screen_name="this.screen_name"></get-source>
                <pre-publication  :notauth="this.readonly" v-on:eventPublicationUpdate="eventPublicationUpdate" v-on:eventPublicationInsert="eventPublicationInsert" :publicationEdit="publicationSelected" :time="timePre" :publication_type=this.publication_type></pre-publication>
                <tuits-list v-on:eventShowLoading="eventShowLoading" v-on:eventPrePublicationTweet="eventPrePublicationTweet" v-on:eventPrePublication="eventPrePublication" :time="timeTuits" :screen_name="this.screen_name"></tuits-list>
                <loading :time="timeLoading" :showLayer="this.showLayer"></loading>
                <notes v-on:eventShowLoading="eventShowLoading" :time="timeNotes" :publicationEdit="publicationSelected" :readonly="this.readonly"></notes>
